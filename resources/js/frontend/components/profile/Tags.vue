  ...
  <div class="bg-white p-6 shadow rounded-lg relative">
    ...
    <form v-if="editing" @submit.prevent="saveTags">
      <div class="flex flex-wrap gap-2 mb-4">
        <div v-for="tag in formData.tags" :key="tag.id" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-full flex items-center">
      ...
      <div class="mb-4">
        ...
        <select id="new-tag" v-model="newTag" class="border p-2 rounded w-full">
    ...
    <div v-else class="flex flex-wrap gap-2">
      <div v-for="tag in business.tags" :key="tag.id" class="bg-gray-200 text-gray-700 px-4 py-2 rounded-full flex items-center">
        <div v-html="getSvgIcon(tag.name)" class="w-6 h-6 mr-2 relative"></div>
        {{ tag.name }}
      </div>
    </div>
...
export default {
  ...
  methods: {
    toggleEdit() {
      ...
      if (this.editing) {
        this.formData.tags = [...this.business.tags];
      }
    },
    cancelEdit() {
      this.editing = false;
    ...
    saveTags() {
      axios.put(`/business/${this.business.slug}/schimba-taguri`, { tags: this.formData.tags.map(tag => tag.name) })
        .then(response => {
          notification.success(response.data.message);
          this.business.tags = this.formData.tags;
          this.editing = false;
        })
        .catch(error => {
